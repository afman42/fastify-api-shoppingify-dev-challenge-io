...
export const createItemHandler = async (
  ...
  try {
    ...
    const resultValidation = createItemValidationSchema.safeParse({
      nama,
      catatan,
      gambar_url,
      idKategori,
...
export const allItemHandler = async (
  _: FastifyRequest,
  reply: FastifyReply
) => {
  try {
...
export const deleteItemHandler = async (
  ...
  try {
    ...
    if (!resultId) {
      return reply
        .code(404)
        .send(jsonMetaAndData(404, "error", { message: "The row not found" }));
    }
...
export const searchItemHandler = async (
  ...
  try {
    const query = (request.query as any).namaItem as string
    const result = await searchNamaItem(query)
    return reply
      .code(200)
      .send(
        jsonMetaAndData(200, "success", result)
      );
  } catch (error) {
    reply.code(500).send(jsonMetaAndData(500, "error", error));
    console.log(error);
