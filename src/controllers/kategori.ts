...
export const createKategoriHandler = async (
  request: FastifyRequest,
  reply: FastifyReply
) => {
  try {
    const namaKategori = (request.body as any).namaKategori as string;
    const resultValidation = createKategoriValidationSchema.safeParse({
      namaKategori,
    });
...
export const updateKategoriHandler = async (
  request: FastifyRequest,
  reply: FastifyReply
) => {
  try {
...
export const deleteKategoriHandler = async (
  request: FastifyRequest,
  reply: FastifyReply
) => {
  try {
...
export const searchNameKategoriHandler = async (
  request: FastifyRequest,
  reply: FastifyReply
) => {
  try {
    const query = (request.query as any).namaKategori as string;
    const result = await searchNamaKategori(query);
    return reply.code(200).send(jsonMetaAndData(200, "success", result));
  } catch (error) {
    reply.code(500).send(jsonMetaAndData(500, "error", error));
    console.log(error);
