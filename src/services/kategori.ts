...
export async function updateKategori(namaKategori: string, id: string) {
  const result = await db
    .update(kategoris)
    .set({ namaKategori, updatedAt: new Date() })
    .where(eq(kategoris.id, parseInt(id)))
    .returning();
  return result[0];
}
...
export async function searchNamaKategori(namaKategori: string = "") {
  let result;
  if(namaKategori != ""){
    result = await db.select().from(kategoris).where(ilike(kategoris.namaKategori,"%" + namaKategori + "%")).limit(3)
  }else {
    result = await db.select().from(kategoris).limit(3)
  }
  return result;
}
